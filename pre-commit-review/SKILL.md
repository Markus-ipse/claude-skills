...
- **Nothing but prose changed** — documentation, comments, copy → native pass alone.
- **Anything that executes or pins a dependency changed** → native pass, Codex pass, and product pass. Source files, obviously, but also CI workflow YAML, Dockerfiles, Terraform and other deployment manifests, and lockfiles. These look inert and are not: a widened permission, an unsafe container setting or a compromised transitive dependency is a security finding, and the coverage map below gives security exactly one owner — the Codex pass. Routing them down the prose path would skip the only reviewer that covers them, which is the opposite of what a change to a deploy manifest deserves.
